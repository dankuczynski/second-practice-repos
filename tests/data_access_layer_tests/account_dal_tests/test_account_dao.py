from customer_exceptions.no_account_found import NoAccountFound
from data_access_layer.account_dao.account_dao import AccountDAOImp
...
def test_select_all_accounts_by_customer_id_no_account():
    try:
...
def test_transfer_sender_account_not_found():
    try:
        ...
        assert str(e) == "sender account no found with given ID"
...
def test_transfer_receiver_account_not_found():
    try:
        ...
        assert str(e) == "receiver account no found with given ID"
